...
export type ClearSignSolanaArgType = 'u64' | 'u8' | 'pubkey' | 'opaque32' | 'lamports'
...
export type ClearSignEventSource = 'studio' | 'vault-rpc' | 'rest-api' | 'walletconnect' | 'vault-swap'
...
export type ClearSignDefinitionSource = 'none' | 'native' | 'runtime' | 'certified' | 'erc7730'
export type ClearSignObservationOutcome = 'pending' | 'signed' | 'rejected' | 'timed-out' | 'policy-blocked' | 'failed'
export type ClearSignObservationKind = 'evm-call' | 'evm-typed-data' | 'solana-transaction'
...
/** Local, privacy-safe denominator for ClearSign coverage metrics. */
export interface ClearSignObservationDraft {
  chain: 'Ethereum' | 'Solana'
  requestKind: ClearSignObservationKind
  shapeKey: string
  /** Public contract/program shape only. Never raw arguments or user accounts. */
  shape: Record<string, unknown>
  protectionLevel: ClearSignProtectionLevel
  definitionSource: ClearSignDefinitionSource
  /** Immutable simulation result; authentication may advance protectionLevel but never changes this evidence. */
  simulationStatus: ClearSignSimulationStatus
  /** Immutable host resolution result. `selected` is not device authentication; P4 records that separately. */
  definitionResolution: ClearSignDefinitionResolution
  /** Privacy-safe potential-loss class. Never stores an amount, balance, account, or asset id. */
  exposureClass: ClearSignExposureClass
}

export interface ClearSignObservation extends ClearSignObservationDraft {
  id: string
  createdAt: number
  finalizedAt?: number
  deviceId?: string
  source: ClearSignEventSource
  outcome: ClearSignObservationOutcome
  errorClass?: string
  /** Number of call/instruction components in the signed request. */
  componentCount?: number
  /** Components whose description was authenticated by the device. */
  authenticatedComponentCount?: number
}

export interface ClearSignCoverageSummary {
  totalRequests: number
  authenticatedRequests: number
  authenticatedPercent: number
  componentCoverage: {
    totalAppearances: number
    authenticatedAppearances: number
    authenticatedPercent: number
    fullyAuthenticatedRequests: number
    partiallyAuthenticatedRequests: number
    unauthenticatedRequests: number
  }
  byLevel: Record<ClearSignProtectionLevel, number>
  byOutcome: Record<ClearSignObservationOutcome, number>
  bySimulation: Record<ClearSignSimulationStatus, number>
  byDefinitionResolution: Record<ClearSignDefinitionResolution, number>
  byExposure: Record<ClearSignExposureClass, number>
  definitions: {
    checkedRequests: number
    selectedRequests: number
    refusedRequests: number
    noArtifactRequests: number
  }
  auditQueue: {
    pending: number
    auditing: number
    ...
    historicalIdentityChanges: number
  }
  simulation: {
  ...
  byChain: Record<ClearSignObservation['chain'], ClearSignCoverageSlice>
  bySource: Record<ClearSignEventSource, ClearSignCoverageSlice>
  rolling: {
    last24Hours: ClearSignCoverageSlice
    last7Days: ClearSignCoverageSlice
  }
  topUnknown: Array<{
    shapeKey: string
    chain: ClearSignObservation['chain']
    count: number
    shape: Record<string, unknown>
    maxExposureClass: ClearSignExposureClass
  }>
  /** Instruction/call frequency across requests; coverage is not implied. */
  topComponents: Array<{
    componentKey: string
    chain: ClearSignObservation['chain']
    count: number
    ...
    maxExposureClass: ClearSignExposureClass
    auditStatus?: 'pending' | 'auditing' | 'evidence-ready' | 'covered' | 'failed' | 'identity-changed'
  }>
}

export interface ClearSignCoverageSlice {
  totalRequests: number
  authenticatedRequests: number
  authenticatedPercent: number
}

export interface ClearSignAuditJob {
  shapeKey: string
  chain: ClearSignObservation['chain']
  shape: Record<string, unknown>
  firstSeenAt: number
  lastSeenAt: number
  requestCount: number
  status: 'pending' | 'auditing' | 'evidence-ready' | 'identity-changed' | 'covered' | 'failed'
  lastProtectionLevel: ClearSignProtectionLevel
  maxExposureClass: ClearSignExposureClass
  attempts: number
  auditStartedAt?: number
  auditCompletedAt?: number
  evidence?: ClearSignAuditEvidence
  errorClass?: string
}

export interface ClearSignCodeIdentityEvidence {
  address: string
  role: 'contract' | 'implementation' | 'beacon' | 'program' | 'program-data'
  codeHash?: string
  owner?: string
  executable?: boolean
  deployedSlot?: string
...
/** Public-chain evidence collected for an unknown request shape. Not a trusted definition. */
export interface ClearSignAuditEvidence {
  version: 1
  auditedAt: number
  chain: ClearSignObservation['chain']
  endpoint: string
  stateReference?: string
  identities: ClearSignCodeIdentityEvidence[]
  candidates?: Array<{
    source: 'sourcify' | 'erc7730-registry' | 'anchor-idl' | 'program-registry'
    address: string
...
export type ClearSignMutationKind = 'truncation' | 'selector-or-discriminator' | 'field-boundary' | 'account-privilege' | 'chain-or-contract'

/** Immutable review input. Raw transactions remain opt-in and external; hashes bind fixtures without retaining user data. */
export interface ClearSignPromotionBundle {
  version: 1
  shapeKey: string
  chain: ClearSignObservation['chain']
  candidate: NonNullable<ClearSignAuditEvidence['candidates']>[number]
  identityHash: string
  createdAt: number
  expiresAt: number
  fixtures: Array<{
    ...
    payloadHash: string
    decodedFieldsHash: string
    effectsHash: string
    provenance: 'user-opt-in' | 'operator-reproduction' | 'public-chain'
  }>
  mutations: Array<{
    id: string
    kind: ClearSignMutationKind
    fixtureHash: string
    expected: 'reject' | 'different-display' | 'different-effect'
    observed: 'reject' | 'different-display' | 'different-effect' | 'unexpected-pass'
  }>
  deviceOracle: {
    firmwareVersion: string
    artifactHash: string
...
export interface ClearSignPromotionReview {
  bundleHash: string
  reviewerPublicKey: string
  role: 'semantics-review' | 'security-review'
  decision: 'approve' | 'reject'
  reviewedAt: number
  /** 65-byte recoverable secp256k1 signature over the domain-separated full review statement. */
  signature: string
}

export interface ClearSignPromotionVerdict {
  bundleHash: string
  publishable: boolean
  failures: string[]
...
export interface ClearSignCentralRequest {
  id: string
  network: string
  contract: string
  selector: string
  ...
  reason: string
  status: 'pending' | 'auditing' | 'awaiting-approval' | 'approved' | 'published' | 'rejected' | 'revoked'
...
export interface ClearSignCentralContractAudit {
  auditId: string
  requestId: string
  evidenceHash: string
  submittedAt: number
  status: string
  shape: { network: string; contract: string; selector: string; calldataLength: number }
  evidence: Record<string, unknown>
}

export interface ClearSignCentralContractReview {
  version: 1
  auditId: string
  evidenceHash: string
  reviewerPublicKey: string
...
export interface ClearSignCentralAssetAudit {
  caip: string
  network: string
  standard: string
  identity: string
  ...
  coverage_status: 'eligible-on-demand' | 'needs-work' | 'denied'
  audit_status: 'pending' | 'auditing' | 'candidate' | 'covered' | 'rejected'
  approval_status: 'pending' | 'approved' | 'rejected'
  audit_attempts: number
  last_audit_error?: string | null
  audit_evidence_hash?: string | null
  current_asset_audit_id?: string | null
  audited_at?: number | null
  next_recheck_at?: number | null
  blockers: string[]
}

export interface ClearSignCentralAssetAuditHistory {
  auditId: string
  caip: string
  evidenceHash: string
  submittedAt: number
...
export interface ClearSignCentralAssetReview {
  version: 1
  caip: string
  evidenceHash: string
  reviewerPublicKey: string
...
export interface ClearSignCentralAssetReviewResult {
  reviewId: string
  caip: string
  auditStatus: string
...
export interface ClearSignCentralStatus {
  available: boolean
  sourceHash?: string
  totalAssets: number
  ...
  byAssetAudit: Record<string, number>
  byAssetApproval: Record<string, number>
  contractQueue: Record<string, number>
  certificateChainIds: number[]
  requests: ClearSignCentralRequest[]
  assetCandidates: ClearSignCentralAssetAudit[]
  reviewPolicy?: {
    version: 1
    ready: boolean
    requiredRoles: string[]
    distinctReviewersRequired: boolean
    reviewers: Array<{ publicKey: string; fingerprint: string; roles: string[] }>
  }
  sourceObservation?: { assetsHash: string; denylistHash: string; observedAt: number; observer: string }
  sourceDrift: boolean
}

export interface ClearSignPromotionRecord {
  bundleHash: string
  shapeKey: string
...
export interface ClearSignProtectionCaseStudy {
  id: string
  chain: 'Ethereum' | 'Solana'
  title: string
  transactionId: string
  status: 'verified' | 'unavailable' | 'contradicted'
  stateReference?: string
  observedAt?: number
  protectionLevel: ClearSignProtectionLevel
  facts: string[]
...
export interface EIP712DecodedInfo {
  ...
  isKnownType: boolean
  /** True only when the signed protocol contract identity is in the reviewed deployment registry. */
  protocolIdentityVerified?: boolean
}
...
export interface SigningRequestInfo {
  ...
  certifiedEvmSchema?: {
    ...
    definitionReview?: import('./clearsign-report').ClearSignDefinitionReview
  }
  ...
  /** Vault-side prediction + authenticated-description boundary for this exact request. */
  clearSignReport?: import('./clearsign-report').ClearSignReport
}
...
export interface AppSettings {
  ...
  evmSimulationRpcUrls: Record<string, string>
}
...
export interface SwapUiState {
  ...
  previewBuild: {
    ...
    clearSignReports?: {
      approval?: import('./clearsign-report').ClearSignReport
      transaction?: import('./clearsign-report').ClearSignReport
    }
  } | null
  error: string | null
  txid: string | null
