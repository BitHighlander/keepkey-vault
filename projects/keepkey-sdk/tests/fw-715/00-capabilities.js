 ...
 *
 * Cheap enough to run on every build. Catches a device older than 7.15,
...
run('fw-715 capabilities — firmware 7.15+ and required policy state', async (getSdk, assert) => {
  ...
  // #367 applies to physical firmware. The emulator reports no installed
  // firmware hash because it runs a host dylib, not a signed device image.
  // Bind emulator QA to the build's source commit separately (see the
  // clear-sign SOP); never treat an empty physical-device hash as valid.
  const hash = f.firmware_hash || ''
  const isEmulator = f.firmware_variant === 'Emulator'
  assert(isEmulator ? 'emulator variant (source commit checked separately)' : 'reports a firmware hash',
    isEmulator || (hash.length > 0 && !/^0+$/.test(hash)))
